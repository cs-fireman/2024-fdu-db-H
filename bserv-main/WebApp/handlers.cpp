...
bserv::db_relation_to_object orm_note{
	bserv::make_db_field<int>("note_id"),
	bserv::make_db_field<int>("user_id"),
	bserv::make_db_field<std::string>("note_content"),
	bserv::make_db_field<std::string>("note_title"),
	bserv::make_db_field<std::string>("update_time")
	// bserv::make_db_field<std::string>("user_name")
};
bserv::db_relation_to_object orm_note_id{
	bserv::make_db_field<int>("note_id")
};
...
boost::json::object update_note(
	...
	if (params.count("noteId") == 0) {
		return {
			{"success", false},
			{"message", "`noteId` is required"}
		};
	}
	if (params.count("noteContent") == 0) {
		return {
			{"success", false},
			{"message", "`noteContent` is required"}
		};
	}
	if (params.count("updateTime") == 0) {
		return {
			{"success", false},
			{"message", "`updateTime` is required"}
		};
	}
	lgdebug << "in update_note" << std::endl;
	std::string params_str = boost::json::serialize(params);
	lginfo << "Received params: " << params_str << std::endl;
	auto noteId = params["noteId"];
	auto noteContent = params["noteContent"].as_string();
	auto updateTime = params["updateTime"].as_string();
	lginfo << noteId << ' ' << noteContent << ' ' << updateTime << std::endl;
	bserv::db_transaction tx{ conn };
	bserv::db_result r = tx.exec(
		"update note "
		"set note_content = ?, update_time = ? "
		"where note_id = ? ; ",
		noteContent,
		updateTime,
		noteId
...
boost::json::object add_note(
	...
	lginfo << "Received params: " << params_str << std::endl;
	auto noteId = params["noteId"];
	auto userId = params["userId"];
	auto noteTitle = params["noteTitle"].as_string();
	auto updateTime = params["updateTime"].as_string();
	auto userName = params["userName"].as_string();
	lginfo << noteId << ' ' << userId << ' ' << noteTitle << ' ' << updateTime << ' ' << userName << std::endl;
	bserv::db_transaction tx{ conn };
	bserv::db_result r = tx.exec(
		"insert into note (user_id, note_content, note_title, update_time)"
	...
	lginfo << r.query();
	auto info = orm_note_id.convert_to_vector(r);
	
	boost::json::array json_info;
	for (auto& i : info) {
		json_info.push_back(i);
	}
	lgdebug << "Final JSON information: " << json_info[0] << std::endl;
...
boost::json::object user_login(
	...
	bserv::session_type& session = *session_ptr;
	session["user"] = user;
	auto user_b = get_user(tx, username.c_str());
	return {
		{"success", true},
...
boost::json::object list_notes(
	std::shared_ptr<bserv::db_connection> conn,
	std::shared_ptr<bserv::session_type> session_ptr,
	const std::string& user_name) {
	lgdebug << "in list notes" << std::endl;
	lgdebug << "user_name " << user_name << std::endl;
	bserv::db_transaction tx{ conn };
	bserv::db_result db_res = tx.exec(
		"select note.note_id, note.user_id, note.note_content, note.note_title,	note.update_time "
		"from note, auth_user "
		"where note.user_id = auth_user.id and auth_user.username = ? ;",
		user_name
	);
	lginfo << db_res.query();
	lgdebug << "hello" << std::endl;
	auto notes = orm_note.convert_to_vector(db_res);	
	boost::json::array json_notes;
	for (auto& note : notes) {
		json_notes.push_back(note);
	}
	lgdebug << "Final JSON Notes: " << json_notes << std::endl;
