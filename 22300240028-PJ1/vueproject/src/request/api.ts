...
//根据userName查询用户notes   get
export const GetUserNotes = (params: { userName: string }): Res<null> =>
    instance.get(`/api/users/list_notes/${params.userName}`, {params});

//根据noteId,noteContent,updateTime更新note  post
export const UpdateNoteContent = (data: { noteId: number, noteContent: string, updateTime: string }): Res<null> =>
    instance.post('/api/update_note', data);

//根据username,noteTitle,noteId增加note  post
export const AddNote = (data: { noteId: number, userId: number, noteTitle: string, updateTime: string, userName: string }): Res<null> =>
    instance.post('/api/add_note', data);

//根据noteId删除note  post
export const DeleteNote = (data: { noteId: number }): Res<null> =>
    instance.post('/api/delete_note', data);
//以下是模板:
// post请求 ，没参数
export const LogoutAPI = (): Res<null> =>
